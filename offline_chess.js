...
function setChessBox() {
    for (let i = 1; i < 9; i++) {
        for (let j = 1; j < 9; j++) {
            ...
            // blackDiv.style.backgroundColor = i%2 == 0 ? (j%2 ==0 ? "rgb(200, 99, 99)" : "antiquewhite") : (j%2 ==0 ? "antiquewhite" : "rgb(200, 99, 99)");
    ...
    boxes.forEach((element) => {
        element.addEventListener("click", () => {
            try {
                ...
                previousClick.style.fontSize = "50px";
            } catch (error) { }
...
function gameOver(winArmy) {
    console.log("Hello");
    dissable();
    let result = document.querySelector("#gameOver");
    result.style.display = "block";
    if (winArmy != null) {
        if (winArmy[3] == white[3]) {
            result.innerText = "White Win !";
        }
        else {
            result.innerText = "Black Win !";
        }
    }
    else {
